#!/bin/bash
sudo npm install orm@3.1.0
sudo npm install mocha@1.12.0
sudo npm install express@3.3.4
sudo npm install amqp@0.2.4
sudo npm install moment@2.1.0
sudo npm install mysql@2.10.2
sudo npm install apn@1.7.6
sudo npm install winston@0.7.2
sudo npm install forever@0.15.1 -g
sudo npm install node-gcm@0.9.6
sudo npm install request@2.34.0
sudo npm install morgan@1.7.0
sudo npm install pug@2.0.0-beta3
